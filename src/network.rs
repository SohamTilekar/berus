...
impl URL {
    pub fn new(url_str: &str) -> Result<Self, Box<dyn Error>> {
    ...
    pub fn request(&self) -> Result<String, Box<dyn Error>> {
        ...
        let mut stream: Box<dyn ReadWrite> = if self.scheme == "https" {
            ...
            let tls_stream = connector.connect(&self.host, tcp)?;
        ...
        if parts.len() < 3 {
            return Err(
                BrowserError(format!("Malformed status line: {}", statusline.trim())).into(),
            );
        }
